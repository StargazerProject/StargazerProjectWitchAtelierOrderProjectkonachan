...
import com.pisual.witchatelier.dao.OrderMongoDBMethodInterFace;
import com.stargazer.witchatelier.model.event.Order;
import com.stargazer.witchatelier.util.system.PropertiesUtil;

public class OrderBackUPMongoDBMethod implements OrderMongoDBMethodInterFace {
	...
	/**日志初始化**/
	static Logger logger = Logger.getLogger(OrderBackUPMongoDBMethod.class.getName());
	/**数据库初始化**/
	static
	{
		try {
			mg = new Mongo(PropertiesUtil.getValue("OrderBackUPCacheMongodbIP"),Integer.parseInt(PropertiesUtil.getValue("OrderBackUPCacheMongodbPort")));
			...
			db = mg.getDB(PropertiesUtil.getValue("OrderBackUPCacheMongodbDataBase"));
			collection = db.getCollection(PropertiesUtil.getValue("OrderBackUPCacheMongodbDataBaseCollection"));
		} catch (Exception e) {
	...
	public void saveOrder(Order order) {
		...
		logger.info("完成存储Order BackUP 受影响的行数: "+num);
	}
	@Override
	public void delateOrder(Order order) {
		...
		logger.info("完成删除Order BackUP 受影响的行数: "+num);
	}
	@Override
	public void changeOrder(Order order) {
		...
		logger.info("完成更新Order BackUP 受影响的行数: "+num.getN());
	}
	/**删除数据库Order表文件**/
	@Override
